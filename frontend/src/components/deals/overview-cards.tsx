...
// Deal Overview cards: the single "ask this deal" bar, Recent meetings,
// Upcoming, and one merged "Latest insights" card (actions + decisions +
// questions). The page owns data fetching and passes plain props down.
...
// The single, prominent "ask this deal" entry point. Clicking jumps into the
// Ask AI tab (where the real composer + scope rail live), so there is exactly
// one ask experience — no duplicated mini-composers scattered across the page.
export function DealAskBar({ dealId }: { dealId: string }) {
  return (
    <div
    ...
    >
      <div
        ...
        style={{
          background:
            "linear-gradient(180deg, var(--ws-ai-tint) 0%, var(--ws-bg) 75%)",
        }}
      >
        <Link
        ...
        >
          ...
          <span className="flex-1 text-[13px]" style={{ color: "var(--ws-muted)" }}>
            Ask anything about this deal — one meeting, several, or all…
          </span>
          <ArrowRight className="w-3.5 h-3.5 shrink-0" style={{ color: "var(--ws-ai-ink)" }} />
        </Link>
        <div className="flex flex-wrap gap-1.5 mt-2.5">
          {SUGGESTED_PROMPTS.map((p) => (
            <Link
              ...
              className="px-2.5 py-1 rounded-full text-[11.5px] leading-snug"
              style={{
                background: "var(--ws-bg)",
            ...
            >
              <Sparkles
                className="w-2.5 h-2.5 align-middle inline mr-1"
...
// One consolidated card for everything AI extracted — a light digest of the
// top action items, decisions, and open questions. The full triage lives on
// the Action Items tab; this is just the "what surfaced" glance on Overview.
export function LatestInsights({
  ...
  const total = actions.length + decisions.length + questions.length;
  const { data: completions } = useActionItemCompletions(dealId);
  const toggle = useToggleActionItem();

  const rowBorder = (first: boolean) =>
    first ? undefined : "1px solid var(--ws-border)";
  ...
  return (
    ...
    >
      {total === 0 ? (
        <div className="px-3.5 py-5 text-[12px]" style={{ color: "var(--ws-muted)" }}>
          No insights yet. Action items, decisions, and open questions surface
          here as meeting analyses complete.
        </div>
      ) : (
        <>
          {topActions.map((a, i) => {
              ...
              >
                ...
                {ownerInitials ? (
                  <span
                    ...
                    style={{ width: 20, height: 20, background: avatarColor(a.owner) }}
                    title={a.owner}
                  >
                    {ownerInitials}
                  </span>
          ...
          {topDecisions.map((d, i) => (
            <div
              ...
              style={{ borderTop: rowBorder(i === 0 && topActions.length === 0) }}
            >
              <CheckCircle2
          ...
          {topQuestions.map((q, i) => (
            <div
              ...
              style={{
                borderTop: rowBorder(
                  i === 0 && topActions.length === 0 && topDecisions.length === 0,
                ),
              }}
            >
              <HelpCircle
